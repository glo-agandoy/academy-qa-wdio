...
const loadEnvFiles = () => {
  ...
  [`.env.${env}`].forEach(file => {
    const filePath = path.join(baseEnvPath, file);

    if (fs.existsSync(filePath)) dotenvConfig({ path: filePath });

    if (!fs.existsSync(filePath)) console.warn(`File: ${filePath} is not defined`);
  });
};
